...
def save_patch_visualizations(
    ...
    """
    Save patch-level visualization artifacts only.

    This function does not extract radiomics features.
    """
...
def save_patch_visualizations_from_h5(
    ...
    """
    Save patch/mask visualizations from a patch H5 file.

    This function is fully separated from radiomics extraction.
    """
...
def save_segment_overlays_from_h5(
    ...
    """
    Save CellViT segmentation overlays.

    This function assumes segmentation results already exist.
    It does not run CellViT.
    """
    ...
    if patch_indices is not None:
        patch_df = patch_df[patch_df[PATCH_IDX_COLUMN].isin(patch_indices)].copy()
        seg_gdf = seg_gdf[seg_gdf[PATCH_IDX_COLUMN].isin(patch_indices)].copy()
    ...
    print(f"[SAVE] overlays saved to {overlay_dir}")
